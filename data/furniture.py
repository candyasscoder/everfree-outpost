...
def init():
    ...
    s = structure_builder()
    s.merge(mk_solid_structure('bed', image, (2, 2, 1), (0, 0), model='bed'))
    ...
    mk_structure_item(s['bookshelf/0'], 'bookshelf', 'Bookshelves', (0, 1)) \
            .recipe('anvil', {'wood': 20})
    mk_solid_structure('bookshelf/1', image, (1, 1, 2), (6, 0), model='cabinet', layer=2)
    mk_solid_structure('bookshelf/2', image, (1, 1, 2), (7, 0), model='cabinet', layer=2)
    ...
    mk_model('stair/n', [tuple(x * TILE_SIZE for x in vert) for vert in stair_verts_tile])

    stair_img = structures('stair.png')
    stair_shape = Shape(1, 1, 1, ['ramp_n'])
    stair = mk_structure('stair/n', stair_img, 'stair/n', stair_shape, 1)
